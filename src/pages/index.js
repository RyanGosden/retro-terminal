...
export default function Home() {
  ...
  const parseCommand = (prompt) => {
    ...
    switch (prompt) {
      ...
      default:
        setOutput([...newOutput, ...[{prompt: `Command '${prompt}' not found`, orig: 'system'}]]);
    }
  }
  ...
  return (
    <div className="screen">
        <div className="output">
          {output.map((out, key) => {
            return <div key={key} className="output_prompt">
     ...
     <div className="input">
      <label className="user">
        {promptPrefix()}
        <input type="text" value={prompt} className="prompt" onChange={handleInput} onKeyDown={handleEvents} autoFocus/>
      </label>
     </div>
    </div>
  )
